 ...
 * <p>
 * statusBar notificationBar 状态栏 屏幕上方电量/时间所在的区域
 * actionBar 活动顶部的titlebar
 * navigationBar 导航栏 屏幕底部的虚拟键
 * <p>
 * statusBar：
...
public final class LBarUtilsX {
    ...
    public static void setNavBarVisibility(@NonNull final Window window, boolean isVisible) {
        ...
        for (int i = 0, count = decorView.getChildCount(); i < count; i++) {
            ...
            if (id != View.NO_ID) {
                String resourceEntryName = LCommon.getApp()
                        .getResources()
    ...
    public static boolean isSupportNavBar() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            WindowManager wm = (WindowManager) LCommon.getApp().getSystemService(Context.WINDOW_SERVICE);
        ...
        boolean menu = ViewConfiguration.get(LCommon.getApp()).hasPermanentMenuKey();
    ...
    private static Activity getActivityByView(@NonNull final View view) {
        ...
        Log.e("LBarUtilsX", "the view's Context is not an Activity.");
        return null;
    }
}
